...
from handlers import (
    greet_user, talk_to_me, guess_number, send_stalker_pict,
    user_coordinates, word_count, full_moon, eph, check_user_photo,
    photo_request
)
...
def main():
    ...
    anketa = ConversationHandler(
      entry_points=[MessageHandler(Filters.regex('^(Заполнить анкету)$'),
                    anketa_start)],
      states={
    ...
    dp.add_handler(MessageHandler(Filters.regex('^(Отправить картинку)$'),
                                  photo_request))
    dp.add_handler(MessageHandler(Filters.photo, check_user_photo))
    dp.add_handler(MessageHandler(Filters.regex('^(Кто здесь)$'),
                                  send_stalker_pict))
